...
window.onload = function () {
  ...
  const mainContent = getEl("main-content"),
    ...
    addTaskButton = getEl("add-task-btn"),
    ...
    googleSignInButton = getEl("google-signin-btn"),
    signOutButton = getEl("signout-btn"),
    confirmModal = getEl("confirm-modal"),
    confirmModalMessage = getEl("confirm-modal-message"),
    confirmDeleteButton = confirmModal.querySelector(".confirm-btn"),
    cancelDeleteButton = confirmModal.querySelector(".cancel-btn"),
    ...
    viewEditModal = getEl("view-edit-modal"),
    editTaskInput = getEl("edit-task-input"),
    saveEditBtn = getEl("save-edit-btn"),
    cancelEditBtn = getEl("cancel-edit-btn"),
    activeTasksList = getEl("active-tasks-list"),
    ...
    deferredDurationInput = getEl("deferred-duration"),
    staleDurationInput = getEl("stale-duration"),
    saveSettingsBtn = getEl("save-settings-btn"),
    settingsMessage = getEl("settings-message"),
    incomingTasksSection = getEl("incoming-tasks-section"),
    ...
    friendNameInput = getEl("friend-name-input"),
    friendIdInput = getEl("friend-id-input"),
    saveFriendBtn = getEl("save-friend-btn"),
    cancelFriendBtn = getEl("cancel-friend-btn"),
    assignTaskModal = getEl("assign-task-modal"),
    friendSelect = getEl("friend-select"),
    assignTaskInput = getEl("assign-task-input"),
    sendTaskBtn = getEl("send-task-btn"),
    cancelAssignBtn = getEl("cancel-assign-btn"),
    chatsBtn = getEl("chats-btn"),
    chatNotificationBadge = getEl("chat-notification-badge"),
    chatSection = getEl("chat-section"),
    backToTasksBtn = getEl("back-to-tasks-btn"),
    chatFriendsList = getEl("chat-friends-list"),
    chatWindow = getEl("chat-window"),
    chatWindowPlaceholder = getEl("chat-window-placeholder"),
    ...
    notificationsStatus = getEl("notifications-status"),
    imageViewerModal = getEl("image-viewer-modal"),
    fullImage = getEl("full-image"),
    ...
    uploadProgressContainer = getEl("upload-progress-container"),
    uploadProgressBar = getEl("upload-progress-bar"),
    ...
    taskDetailsContent = getEl("task-details-content"),
    ...
    resourcesGrid = getEl("resources-grid"),
  ...
  const boardsBtn = getEl("boards-btn"),
  ...
  let userId = null,
    currentAction = null,
    deferredDurationHours = 24,
    staleDurationHours = 72,
    currentTaskToEdit = null,
    selectedFriendForChat = null,
    unsubscribeFromChat = null,
    friendsCache = {},
    shownNotifications = new Set(),
    unsubscribeFromReport = null,
    currentReportTaskIdToDelete = null,
    currentIncomingTask = null,
    currentTaskForAction = null,
    unsubscribeFromResources = null,
    currentShelfIdForResource = null,
    currentShelvesData = [],
    unsubscribeFromBoards = null,
    unsubscribeFromBoardItems = null,
    currentBoardId = null,
    selectedStickerColor = "bg-yellow-200",
    currentMobileTab = "tasks";
  ...
  const getDisplayNameFor = (uid) => {
    ...
    if (friendsCache[uid] && friendsCache[uid].name)
      return friendsCache[uid].name;
  ...
  const loadSettings = async () => {
    ...
    try {
      const settingsRef = doc(
        db,
        `artifacts/${appId}/users/${userId}/settings/app_settings`
      );
      ...
      if (docSnap.exists()) {
        ...
        if (settings.expandedSections) {
          Object.entries(settings.expandedSections).forEach(
            ([key, isExpanded]) => {
              const list =
                getEl(`${key}-tasks-list`) || getEl(`${key}-content`);
              const btn = getEl(`${key}-toggle-btn`);
              if (list && btn) {
                list.classList.toggle("hidden", !isExpanded);
                btn.textContent = isExpanded ? "Згорнути" : "Розгорнути";
              }
            }
          );
        }
      ...
      (querySnapshot) => {
        ...
        querySnapshot.forEach(async (docSnap) => {
          ...
          if (task.status === "deferred" && task.deferredAt) {
            if (
              Date.now() - task.deferredAt.toMillis() >=
              deferredDurationHours * 3600000
            ) {
              await updateTaskStatus(
                task.id,
                "active",
                task.originalSharedTaskId
              );
              showPushNotification("Завдання повернуто до активних", {
                body: `Завдання: "${task.text}"`
              });
              return;
            }
          }
          if (task.status === "active" && task.lastUpdatedAt) {
            if (
              Date.now() - task.lastUpdatedAt.toMillis() >=
                staleDurationHours * 3600000 &&
              !task.isStale
            ) {
              await updateDoc(task.ref, { isStale: true });
              task.isStale = true;
              showPushNotification("Завдання стало застарілим", {
                body: `Зверніть увагу на завдання: "${task.text}"`
              });
            }
          }
  ...
  const renderTasks = (tasks, listElement, status) => {
    ...
    if (!tasks || tasks.length === 0) {
      const messages = {
        active: "Немає активних завдань.",
        deferred: "Немає завдань в роботі.",
        completed: "Немає виконаних завдань."
      };
    ...
    tasks.forEach((task) => {
      ...
      taskItem.className = `p-4 rounded-xl shadow-md flex items-start justify-between ${
        task.isStale ? "bg-red-100" : "bg-white"
      } space-x-2`;
      let buttonsHTML =
        status === "completed"
      ...
      if (status !== "completed") {
        ...
        if (moreBtn)
          moreBtn.addEventListener("click", (e) => {
            e.stopPropagation();
            showTaskActionsModal(task);
          });
      }
      taskItem
        .querySelector("p")
        .addEventListener("click", () => showViewEditModal(task));
      if (taskItem.querySelector(".complete-btn"))
        taskItem
          .querySelector(".complete-btn")
          .addEventListener("click", () =>
            updateTaskStatus(task.id, "completed", task.originalSharedTaskId)
          );
      if (taskItem.querySelector(".active-btn"))
        taskItem
          .querySelector(".active-btn")
          .addEventListener("click", () =>
            updateTaskStatus(task.id, "active", task.originalSharedTaskId)
          );
    ...
    try {
      ...
      if (originalSharedTaskId) {
        const publicTaskRef = doc(
          ...
          "data",
          "shared_tasks",
          originalSharedTaskId
        );
        let publicStatus =
          newStatus === "active" || newStatus === "deferred"
            ? "accepted"
            : newStatus;
  ...
  const prolongTask = async (id) => {
    ...
    try {
      ...
      console.error("Error prolonging task:", e);
    }
  };
  ...
  const saveSettings = async () => {
    ...
    if (
      ...
      settingsMessage.textContent = "Будь ласка, введіть дійсні числа.";
      settingsMessage.className = "text-sm text-center text-red-600";
      return;
    }
    try {
      await setDoc(
        doc(db, `artifacts/${appId}/users/${userId}/settings/app_settings`),
        { deferredDurationHours: newDeferred, staleDurationHours: newStale },
        { merge: true }
      );
      deferredDurationHours = newDeferred;
      staleDurationHours = newStale;
      settingsMessage.textContent = "Налаштування збережено.";
      settingsMessage.className = "text-sm text-center text-green-600 visible";
  ...
  const showConfirmModal = (action, message, buttonText) => {
  ...
  const deleteTask = async (task) => {
    ...
    try {
      await deleteDoc(
        doc(db, `artifacts/${appId}/users/${userId}/tasks`, task.id)
      );
      ...
      console.error("Error processing task deletion: ", e);
      showNotification(
        "Помилка",
        "Не вдалося повністю видалити завдання. Спробуйте ще раз."
      );
    }
  };
  ...
  const showViewEditModal = (task) => {
    currentTaskToEdit = task;
    editTaskInput.value = task.text;
    viewEditModal.classList.remove("hidden");
  };
  const hideViewEditModal = () => {
    viewEditModal.classList.add("hidden");
    currentTaskToEdit = null;
  };
  const saveEditedTask = async () => {
    ...
    if (newText === "") {
      showNotification("Помилка", "Текст завдання не може бути порожнім.");
      return;
    }
    try {
      await updateDoc(
        ...
        { text: newText, lastUpdatedAt: serverTimestamp() }
      );
      hideViewEditModal();
    } catch (e) {
  ...
  const saveFriend = async () => {
    ...
    try {
      await setDoc(
        doc(db, `artifacts/${appId}/users/${userId}/friends`, friendId),
  ...
  const deleteFriend = async (friendId) => {
    ...
    try {
      ...
      showNotification("Помилка", "Не вдалося видалити друга.");
    }
  };
  ...
  const subscribeToFriends = () => {
    ...
    try {
      onSnapshot(
        ...
        (snapshot) => {
          friendsCache = {};
          snapshot.docs.forEach((d) => (friendsCache[d.id] = d.data()));
          renderFriends(Object.values(friendsCache));
          renderChatFriends(Object.values(friendsCache));
        },
        (error) => console.error("Error in friends subscription:", error)
      );
    } catch (e) {
      console.error("Error subscribing to friends:", e);
    }
  };

  const renderFriends = (friends) => {
    friendsList.innerHTML = "";
    friendSelect.innerHTML = '<option value="">Виберіть друга</option>';
    if (friends.length === 0) {
      friendsList.innerHTML = `<p class="text-gray-500 text-center italic">Немає друзів.</p>`;
      return;
    }
    friends.forEach((friend) => {
      const friendItem = document.createElement("div");
      friendItem.className =
        "p-3 rounded-lg bg-white shadow-md flex justify-between items-center";
      ...
      friendItem
        ...
        .addEventListener("click", () => {
          showConfirmModal(
            ...
            `Ви впевнені, що хочете видалити ${friend.name} зі списку друзів?`,
            "Так, видалити"
          );
        });
      friendsList.appendChild(friendItem);
      friendSelect.innerHTML += `<option value="${friend.userId}">${friend.name}</option>`;
    });
  };

  const renderChatFriends = (friends) => {
    chatFriendsList.innerHTML =
      friends.length === 0
        ? `<p class="text-gray-500 text-center italic p-4">Немає друзів для чату.</p>`
        : "";
    friends.forEach((friend) => {
      const friendItem = document.createElement("div");
      friendItem.className =
        "p-3 rounded-lg hover:bg-indigo-100 cursor-pointer flex items-center justify-between";
      friendItem.innerHTML = `<div class="font-semibold text-gray-800 flex-grow">${friend.name}</div><span id="unread-count-${friend.userId}" class="px-2 py-1 bg-red-600 text-white text-xs rounded-full hidden"></span>`;
      friendItem.addEventListener("click", () => selectFriendForChat(friend));
  ...
  const sendAssignedTask = async () => {
    ...
    try {
      const sharedTasksCollection = collection(
        db,
        "artifacts",
        appId,
        "public",
      ...
      await addDoc(sharedTasksCollection, {
        text: taskText,
        senderId: userId,
        senderName: auth.currentUser.displayName || "Анонім",
        recipientId,
        createdAt: serverTimestamp(),
        status: "pending"
      });
      const selectedFriend = {
        userId: recipientId,
        name: friendSelect.options[friendSelect.selectedIndex].text
      };
      await sendChatMessage({
        text: `Я призначив вам завдання: "${taskText}"`,
        recipient: selectedFriend,
        isTask: true
      });
      hideAssignTaskModal();
    } catch (e) {
      console.error("Error sending assigned task:", e);
    }
  };
  ...
  const subscribeToIncomingTasks = () => {
    ...
    onSnapshot(
      ...
      (snapshot) => {
        ...
        renderIncomingTasks(tasks);
      },
      (error) => console.error("Error in incoming tasks subscription:", error)
    );
  };
  ...
  const renderIncomingTasks = (tasks) => {
    ...
    if (tasks.length === 0) {
      incomingTasksSection.style.display = "none";
      return;
    }
    ...
    tasks.forEach((task) => {
      const taskItem = document.createElement("div");
      taskItem.className =
        "p-4 rounded-xl shadow-md bg-white flex items-center justify-between space-x-2";
      taskItem.innerHTML = `<div class="flex-grow cursor-pointer min-w-0"><p class="text-sm text-gray-500">Від: <span class="font-semibold">${task.senderName}</span></p><p class="text-gray-800 break-words">${task.text}</p></div><div class="flex-shrink-0"><button class="view-incoming-task-btn px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold btn-frosty">Переглянути</button></div>`;
      taskItem
        .querySelector(".view-incoming-task-btn")
        .addEventListener("click", () => showViewIncomingTaskModal(task));
      taskItem
        .querySelector(".flex-grow")
        .addEventListener("click", () => showViewIncomingTaskModal(task));
      incomingTasksList.appendChild(taskItem);
    });
  };
  ...
  const acceptIncomingTask = async (task) => {
    ...
    try {
      await addDoc(collection(db, `artifacts/${appId}/users/${userId}/tasks`), {
        ...
        lastUpdatedAt: serverTimestamp(),
        isStale: false,
        assignedBy: task.senderName,
        originalSharedTaskId: task.id
      });
  ...
  const updateSharedTaskStatus = async (taskId, status) => {
    try {
      const taskRef = doc(
        db,
        "artifacts",
      ...
      console.error(`Error updating shared task to ${status}:`, e);
    }
  };
  ...
  const closeChatView = () => {
    ...
    chatWindowPlaceholder.classList.remove("hidden");
  };
    ...
    unsubscribeFromChat = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.docs.forEach((d) =>
          renderChatMessage({ id: d.id, ...d.data() })
        );
        setTimeout(() => {
          ...
          if (message.readBy && !message.readBy.includes(userId)) {
            ...
            if (friendId) {
              ...
              if (!shownNotifications.has(message.id)) {
                ...
                showPushNotification(`Нове повідомлення від ${friendName}`, {
                  body: message.text || "[Зображення]"
                });
  ...
  const renderChatMessage = (message) => {
    ...
    bubble.className = `message-bubble p-3 rounded-lg max-w-xs md:max-w-md break-words relative group ${
      isSentByMe ? "bg-indigo-500 text-white" : "bg-gray-200 text-gray-800"
    }`;
    if (message.isTask) {
      bubble.className =
        "message-bubble p-3 rounded-lg max-w-xs md:max-w-md break-words relative bg-yellow-200 text-yellow-800 border border-yellow-300 text-sm italic mx-auto";
    ...
    if (isSentByMe && !message.isTask) {
      ...
      deleteBtn.className =
        "delete-msg-btn absolute top-1/2 -left-8 transform -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-full bg-gray-300 hover:bg-red-500 text-white";
      deleteBtn.innerHTML =
  ...
  const sendChatMessage = async (options) => {
    ...
    try {
      await addDoc(
        collection(db, "artifacts", appId, "public", "data", "chat_messages"),
        {
          participants: [userId, recipient.userId].sort(),
          text: messageText,
          senderId: userId,
  ...
  const uploadImageAndSendMessage = (file) => {
    ...
    const storageRef = ref(
      storage,
      `chats/${participants.join("_")}/${Date.now()}-${file.name}`
    );
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          sendChatMessage({
            recipient: selectedFriendForChat,
  ...
  const checkNotificationStatus = () => {
    if (!("Notification" in window)) {
      notificationsStatus.textContent = "Браузер не підтримує сповіщення.";
    ...
    notificationsStatus.textContent =
      permission === "granted"
        ...
        : "Надайте дозвіл.";
    enableNotificationsBtn.disabled = permission !== "default";
  };
  const requestNotificationPermission = async () => {
    const permission = await Notification.requestPermission();
    if (permission === "granted") {
      new Notification("Дякуємо!", {
        body: "Тепер ви будете отримувати сповіщення."
      });
    }
  ...
  const closeReportView = () => {
    ...
    mainContent.classList.remove("hidden");
    if (unsubscribeFromReport) unsubscribeFromReport();
    unsubscribeFromReport = null;
  };
  const subscribeToReport = () => {
    ...
    const q = query(sharedTasksCollection, where("senderId", "==", userId));
    unsubscribeFromReport = onSnapshot(
      q,
      (snapshot) => {
        const tasks = snapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
        tasks.sort(
      ...
      (error) => console.error("Error in report subscription:", error)
    );
  };
  const renderReport = (tasks) => {
    reportTableBody.innerHTML = "";
    if (Object.keys(friendsCache).length === 0 && tasks.length > 0) {
      setTimeout(() => renderReport(tasks), 100);
      return;
    }
    const tasksByRecipient = {};
    tasks.forEach((task) => {
      if (!tasksByRecipient[task.recipientId])
        tasksByRecipient[task.recipientId] = [];
      tasksByRecipient[task.recipientId].push(task);
    });
    if (Object.keys(tasksByRecipient).length === 0) {
      reportTableBody.innerHTML = `<tr><td colspan="5" class="text-center text-gray-500 italic py-8">Ви ще не призначили жодного завдання.</td></tr>`;
      return;
    }
    for (const recipientId in tasksByRecipient) {
      const userTasks = tasksByRecipient[recipientId];
      const friendName =
        friendsCache[recipientId]?.name ||
        `Невідомий (${recipientId.substring(0, 5)}...)`;
      const row = document.createElement("tr");
      const statuses = ["pending", "accepted", "completed", "declined"];
      let cellsHTML = statuses
        .map((status) => {
          const tasksInStatus = userTasks.filter((t) => t.status === status);
          const cardsHTML = tasksInStatus
            .map(
              (task) =>
                `<div class="report-task-card p-2 mb-2 bg-white rounded-md shadow-sm border" data-task-text="${escape(
                  task.text
                )}" data-task-id="${task.id}"><p class="text-sm break-words">${
                  task.text
                }</p></div>`
            )
            .join("");
  ...
  const showTaskDetailsModal = (taskText, taskId) => {
    taskDetailsContent.textContent = unescape(taskText);
    currentReportTaskIdToDelete = taskId;
  ...
  const deleteSharedTaskFromReport = async () => {
    if (!currentReportTaskIdToDelete) return;
    try {
      await deleteDoc(
        doc(
          db,
          ...
          "data",
          "shared_tasks",
          currentReportTaskIdToDelete
        )
      );
      hideTaskDetailsModal();
    } catch (e) {
      console.error("Error deleting task from report: ", e);
    }
  };

  const showViewIncomingTaskModal = (task) => {
    currentIncomingTask = task;
    incomingTaskSender.textContent = task.senderName;
    incomingTaskContent.textContent = task.text;
    viewIncomingTaskModal.classList.remove("hidden");
  };
  const hideViewIncomingTaskModal = () => {
    viewIncomingTaskModal.classList.add("hidden");
    currentIncomingTask = null;
  };
  const showTaskActionsModal = (task) => {
    currentTaskForAction = task;
    actionDeferBtn.classList.toggle("hidden", task.status === "deferred");
    actionActivateBtn.classList.toggle("hidden", task.status !== "deferred");
    actionProlongBtn.classList.toggle("hidden", !task.isStale);
    taskActionsModal.classList.remove("hidden");
  };
  const hideTaskActionsModal = () => {
  ...
  const subscribeToResources = () => {
    ...
    unsubscribeFromResources = onSnapshot(
      q,
      (snapshot) => {
        currentShelvesData = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data()
        }));
  ...
  const filterAndRenderResources = () => {
    ...
    const filteredShelves = currentShelvesData
      .map((shelf) => {
  ...
  const renderResources = (shelves) => {
    ...
    if (shelves.length === 0) {
      const emptyMessage = resourcesSearchInput.value
        ? "Нічого не знайдено за вашим запитом."
        : `<div class="col-span-full text-center py-12 text-gray-500"><div class="text-4xl mb-2">📚</div><p>У вас поки немає полиць з ресурсами.</p><p class="text-sm">Створіть першу, натиснувши "Нова Полиця"!</p></div>`;
    ...
    shelves.forEach((shelf) => {
      ...
      const itemsHTML = (shelf.items || [])
        ...
        .join("");
      shelfDiv.innerHTML = `<div class="flex justify-between items-center mb-4 border-b pb-2 border-blue-200"><h3 class="font-bold text-lg text-gray-800 truncate" title="${
        shelf.title
      }">${
        shelf.title
      }</h3><button class="delete-shelf-btn text-gray-400 hover:text-red-500 transition-colors" data-id="${
        shelf.id
      }"><i class="fas fa-trash-alt"></i></button></div><div class="flex-grow overflow-y-auto max-h-64 mb-4 space-y-1 pr-1 custom-scrollbar">${
        itemsHTML ||
        '<p class="text-sm text-gray-400 italic text-center mt-4">Порожньо</p>'
      }</div><button class="add-item-btn w-full py-2 rounded-lg border-2 border-dashed border-indigo-300 text-indigo-500 hover:bg-indigo-50 hover:border-indigo-500 transition-all font-medium text-sm" data-id="${
        shelf.id
      ...
      shelfDiv
        ...
        .addEventListener("click", () =>
          showConfirmModal(
            ...
            `Видалити полицю "${shelf.title}" та всі її ресурси?`,
            "Видалити"
          )
        );
      shelfDiv
        .querySelector(".add-item-btn")
        .addEventListener("click", () => openAddResourceModal(shelf.id));
      shelfDiv.querySelectorAll(".delete-resource-btn").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          const itemId = e.currentTarget.dataset.itemId;
          const originalShelf = currentShelvesData.find(
            (s) => s.id === shelf.id
          );
          if (originalShelf)
            deleteResource(shelf.id, itemId, originalShelf.items);
        });
      });
  ...
  const addShelf = async () => {
    const title = shelfNameInput.value.trim();
    if (!title || !userId) return;
    try {
      await addDoc(
        collection(db, `artifacts/${appId}/users/${userId}/resources`),
        { title: title, createdAt: serverTimestamp(), items: [] }
      );
      addShelfModal.classList.add("hidden");
      shelfNameInput.value = "";
    } catch (e) {
      console.error("Error adding shelf: ", e);
    }
  };
  const deleteShelf = async (shelfId) => {
    ...
    try {
      await deleteDoc(
        doc(db, `artifacts/${appId}/users/${userId}/resources`, shelfId)
      );
    } catch (e) {
      console.error("Error deleting shelf: ", e);
    }
  };
  const openAddResourceModal = (shelfId) => {
    currentShelfIdForResource = shelfId;
  ...
  const addResource = async () => {
    ...
    if (!title || !content) {
      showNotification("Помилка", "Всі поля повинні бути заповнені");
      return;
    }
    ...
    try {
      const shelfRef = doc(
        db,
        `artifacts/${appId}/users/${userId}/resources`,
        currentShelfIdForResource
      );
      await updateDoc(shelfRef, { items: arrayUnion(newItem) });
      addResourceModal.classList.add("hidden");
    } catch (e) {
      console.error("Error adding resource: ", e);
    }
  };
  const deleteResource = async (shelfId, itemId, currentItems) => {
    if (!userId) return;
    try {
      ...
      await updateDoc(shelfRef, { items: updatedItems });
    } catch (e) {
  ...
  const switchMobileTab = (tab) => {
    ...
    if (tab === "tasks") {
      ...
      mobileTabStickers.classList.remove(
        "bg-white",
        "shadow-sm",
        "text-indigo-600"
      );
      ...
      mobileTabStickers.classList.add(
        "bg-white",
        "shadow-sm",
        "text-indigo-600"
      );
      mobileTabStickers.classList.remove("text-gray-500");
      mobileTabTasks.classList.remove(
        "bg-white",
        "shadow-sm",
        "text-indigo-600"
      );
  ...
  function addEditSubtaskRow(text = "") {
    ...
    row
      .querySelector(".remove-subtask-btn")
      .addEventListener("click", () => row.remove());
    container.appendChild(row);
  }

  getEl("addEditSubtaskBtn").addEventListener("click", () =>
    addEditSubtaskRow("")
  );
  ...
  const renderBoardTask = (item) => {
    ...
    if (deleteBtn)
      deleteBtn.addEventListener("click", () =>
        deleteBoardItem_withLogging(item.id)
      );
  ...
  const renderBoardSticker = (item) => {
    ...
    el.innerHTML = `
        ...
        `;
    el.querySelector(".delete-sticker-btn").addEventListener("click", () =>
      deleteBoardItem_withLogging(item.id)
    );
  ...
  const subscribeToBoardItems = (boardId) => {
    ...
    unsubscribeFromBoardItems = onSnapshot(
      q,
      (snapshot) => {
  ...
  const formatActivityText = (act) => {
    ...
    switch (act.type) {
      case "task_added":
        return `${actor} створив завдання: «${
          act.text || act.itemText || "Без назви"
        }»`;
      case "subtask_checked":
        return `${actor} викреслив пункт: «${act.subtaskText || "…"}»`;
      case "subtask_unchecked":
        return `${actor} відновив пункт: «${act.subtaskText || "…"}»`;
      case "sticker_added":
        return `${actor} додав стікер: «${act.stickerText || "…"}»`;
      case "sticker_removed":
        return `${actor} видалив стікер: «${act.stickerText || "…"}»`;
      case "task_deleted":
        return `${actor} видалив елемент`;
      default:
  ...
  const subscribeToBoardActivities = (boardId) => {
    ...
    unsubscribeFromBoardActivities = onSnapshot(
      q,
      (snapshot) => {
        latestBoardActivities = snapshot.docs
  ...
  const populateFilterUsers = (activities) => {
    ...
    boardReportFilterUser.innerHTML =
  ...
  const renderBoardReport = (activities) => {
    ...
    if (filterUser)
      filtered = filtered.filter((a) => a.performedBy === filterUser);
    ...
    if (search)
      filtered = filtered.filter((a) => {
        ...
        return (
          ...
          getDisplayNameFor(a.performedBy).toLowerCase().includes(search) ||
      ...
      .forEach((dayKey) => {
        ...
        grouped[dayKey]
          ...
          .forEach((act) => {
            ...
            title.innerHTML = `<div class="timeline-badge">${formatActivityText(
              act
            )}</div>`;
  ...
  const openBoardReport = () => {
    if (!currentBoardId) {
      showNotification("Помилка", "Дошка не вибрана");
      return;
    }
    boardReportTitle.textContent = `Звіт — ${
      activeBoardTitle.textContent || "Дошка"
    }`;
  ...
  boardReportClose.addEventListener("click", () => {
    ...
    if (unsubscribeFromBoardActivities) {
      unsubscribeFromBoardActivities();
      unsubscribeFromBoardActivities = null;
    }
  });

  boardReportFilterUser.addEventListener("change", () =>
    renderBoardReport(latestBoardActivities)
  );
  boardReportFilterType.addEventListener("change", () =>
    renderBoardReport(latestBoardActivities)
  );
  boardReportSearch.addEventListener(
    "input",
    debounce(() => renderBoardReport(latestBoardActivities), 250)
  );
  ...
  const addBoardTask_withLogging = async () => {
    ...
    try {
      ...
      subtasksContainer.innerHTML =
        '<input type="text" placeholder="Крок 1" class="subtask-input w-full px-3 py-1.5 rounded border border-gray-200 text-sm">';
    } catch (e) {
  ...
  const toggleSubtask_withLogging = async (item, idx, isChecked) => {
    try {
      ...
      await updateDoc(
        doc(db, "artifacts", appId, "public", "data", "board_items", item.id),
        {
  ...
  const subscribeToBoards = () => {
    ...
    unsubscribeFromBoards = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((docSnap) => {
        ...
        const el = document.createElement("div");
        el.className =
          "bg-white p-6 rounded-xl shadow-lg border border-orange-100 cursor-pointer hover:shadow-xl transition-all transform hover:-translate-y-1 relative group";
        ...
        el.innerHTML = `
                    <h3 class="font-bold text-xl text-gray-800 mb-2 pr-6">${
                      board.title
                    }</h3>
                    <p class="text-sm text-gray-500"><i class="fas fa-user-friends"></i> ${
        ...
        el.addEventListener("click", (e) => {
          if (
            e.target.closest(".delete-board-btn") ||
            e.target.closest(".edit-board-btn")
          )
            return;
        ...
        if (isOwner && el.querySelector(".delete-board-btn")) {
          el.querySelector(".delete-board-btn").addEventListener(
            "click",
            (e) => {
              e.stopPropagation();
              showConfirmModal(
                () => deleteBoard(board.id),
                `Видалити дошку "${board.title}"? Це видалить всі завдання та стікери на ній.`,
                "Видалити"
              );
            }
          );
  ...
  const deleteBoard = async (boardId) => {
    ...
    try {
      ...
      batch.delete(
        doc(db, "artifacts", appId, "public", "data", "boards", boardId)
      );
      await batch.commit();
  ...
  const openActiveBoard = (board) => {
    ...
    (function addBoardReportButton() {
      try {
        const header = document.querySelector("#active-board-title")
          ?.parentElement;
        if (!header) return;
        ...
        btn.className =
          "px-3 py-1 rounded bg-indigo-100 text-indigo-700 hover:bg-indigo-200 text-sm";
  ...
  const addBoardMember = async () => {
    ...
    try {
      await updateDoc(
        doc(db, "artifacts", appId, "public", "data", "boards", currentBoardId),
  ...
  const toggleSection = (listEl, btnEl) => {
    listEl.classList.toggle("hidden");
    btnEl.textContent = listEl.classList.contains("hidden")
      ? "Розгорнути"
      : "Згорнути";
    saveExpansionState();
  };
  deferredToggleBtn.addEventListener("click", () =>
    toggleSection(deferredTasksList, deferredToggleBtn)
  );
  completedToggleBtn.addEventListener("click", () =>
    toggleSection(completedTasksList, completedToggleBtn)
  );
  settingsToggleBtn.addEventListener("click", () =>
    toggleSection(settingsContent, settingsToggleBtn)
  );
  friendsToggleBtn.addEventListener("click", () =>
    toggleSection(friendsContent, friendsToggleBtn)
  );
  ...
  imageUploadInput.addEventListener("change", (e) => {
    if (e.target.files[0]) uploadImageAndSendMessage(e.target.files[0]);
  });
  ...
  actionDeferBtn.addEventListener("click", () => {
    if (currentTaskForAction) {
      updateTaskStatus(
        currentTaskForAction.id,
        "deferred",
        currentTaskForAction.originalSharedTaskId
      );
  ...
  addShelfBtn.addEventListener("click", () => {
    shelfNameInput.value = "";
    addShelfModal.classList.remove("hidden");
  });
  ...
  const handleSendMessage = () => {
    if (!selectedFriendForChat) {
      showNotification(
        "Помилка",
        "Будь ласка, спочатку виберіть друга зі списку."
      );
      return;
    }
    sendChatMessage({
  ...
  addBoardMemberBtn.addEventListener("click", () =>
    addBoardMemberModal.classList.remove("hidden")
  );
  cancelBoardMemberBtn.addEventListener("click", () =>
    addBoardMemberModal.classList.add("hidden")
  );
  ...
  addSubtaskFieldBtn.addEventListener("click", () => {
    ...
    input.className =
      "subtask-input w-full px-3 py-1.5 rounded border border-gray-200 text-sm";
    subtasksContainer.appendChild(input);
  });

  addStickerBtn.addEventListener("click", () => {
    stickerTextInput.value = "";
    addStickerModal.classList.remove("hidden");
  });
  ...
  mobileTabStickers.addEventListener("click", () =>
    switchMobileTab("stickers")
  );
  ...
  document.querySelectorAll(".color-option").forEach((el) => {
    el.addEventListener("click", (e) => {
      document
        ...
        .forEach((c) => c.classList.remove("border-gray-600"));
